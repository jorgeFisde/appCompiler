...
const reWhitespaces = /^\s+/; //    createParserState :: x -> s -> ParserState e a s
...
const digit = new Parser(function digit$state(state) {
  ...
  if (target.length > index) {
    return target.length && target[index] && reDigit.test(target[index]) ? updateParserState(state, target[index], index + 1) : updateError(state, `ParseError (position ${index}): Expecting digit, got '${target[index]}'`);
  }
...
exports.digit = digit;
const digits = regex(reDigits).errorMap((_, index) => `ParseError (position ${index}): Expecting digits`); //           letter :: Parser e Char s
...
const letter = new Parser(function letter$state(state) {
  ...
  if (target.length > index) {
    return target.length && target[index] && reLetter.test(target[index]) ? updateParserState(state, target[index], index + 1) : updateError(state, `ParseError (position ${index}): Expecting letter, got '${target[index]}'`);
  }
...
const letters = regex(reLetters).errorMap((_, index) => `ParseError (position ${index}): Expecting letters`); //           anyOfString :: String -> Parser e Char s
...
const whitespace = regex(reWhitespaces) // Keeping this error even though the implementation no longer uses many1. Will change it to something more appropriate in the next major release.
.errorMap((_, index) => `ParseError 'many1' (position ${index}): Expecting to match at least one value`); //           optionalWhitespace :: Parser e String s

exports.whitespace = whitespace;
const optionalWhitespace = possibly(whitespace).map(x => x || ''); //           recursiveParser :: (() => Parser e a s) -> Parser e a s
